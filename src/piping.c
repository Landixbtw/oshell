...
int pipe_redirection(char **args)
{
     ...
     * a way to track the amount of pipes we have for later redirection,
     * tracking of the multiple pipe positions, and the spliting of the commands
     * */
    int pipes_amount = 0;
    ...
    if (pipes_amount == 0)
        return -1; // no pipes found, however
     ...
     * `pipe_redirection()` from the original version in your repository:
    ...
    for (cmd_id = 0; cmd_id <= pipes_amount; cmd_id++) {
        ...
        size_t path_length =
            strlen("/usr/bin/") + strlen(commands[cmd_id][0]) + 1;
        ...
        // for (int i = 0; i < cmd_arg_count; i++) {
        //     fprintf(stderr, "command[%i] -> [%i]: %s -- pointer: %p -- hex
        //     dump: ",
        //             cmd_id, i, commands[cmd_id][i],
        //             (void*)commands[cmd_id][i]);
        //     print_hex_dump(commands[cmd_id][i], strlen(commands[cmd_id][i]) +
        //     1);
        // }
    }
     ...
     * open for writing ouput of fd[1] is input for fd[0].
     *
