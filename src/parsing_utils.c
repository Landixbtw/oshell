...
/*
 * This function takes in the an array of strings, with the start_arg
 * representing the word [i]. The start_pos representing the char [j] aswell as
 * the end_arg and end_pos.
 *
 * This should loop from start_arg to start_pos, then determine the start_j and
 * end_j. We also need to calculate the length needed for the new string,
 * therefore we just count the amount of characters.
 *
...
// NOTE: Reason I am passing the array, and not just one string at a time is
// because I am saving the quote, and doing this with passing a string would
// mean I have to safe the quote char globally no? this way it can be saved in
// the function, because we execute the function once NOTE: A variable called
// whose value was assinged to remove_quotes has to be freed
char **remove_quotes(char **arg)
{
    ...
    while (arg && arg[arg_count])
        arg_count++;
    // in this case calloc is being used, because there are cases where it is
    // being accessed without having a value there causing it to crash.
    char **tmp_args = calloc(arg_count + 1, sizeof(char *));
    if (tmp_args == NULL)
        perror("tmp_args memory allocation failed ");
    ...
    for (int i = 0; i < arg_count; i++) {
        ...
        for (int j = 0; j < strlen(arg[i]); j++) {
            if ((arg[i][j] == '"' || arg[i][j] == '\'')) {
                ...
                if (found_closing) {
                    ...
                    fprintf(stderr,
                            "Warning: No closing quote found for quote "
                            "starting at arg[%d][%d]\n",
                            start_arg, start_pos);
                    break;
                }
        ...
        /*
         * TODO: / NOTE
         * Since the array is shorther, because the string in between the quotes
         * is now a single string and not multiple different ones, the string is
         * terminated early basically looks like this. [echo] [foo bar baz]
         * [NULL] [NULL] [|] [wc] [-w] instead of [echo] [foo]         [bar]
         * [baz]  [|] [wc] [-w] This means the string has to be shortened and
         * everything has to move by x amount to the left
         * - will memmove do the trick?
         * */

        /* THIS IS FOR THE PROJECT REPORT DETAILED EXAMPLE OF OUTPUT AT THIS
        STATE
        ...
        /*
         * NOTE:
         * This !found_quote is working just fine but building the string
         * together, with the quote string does not seem to work.
         * */
        ...
        if (!found_quote) {
            ...
            // for %s failed", tmp_args[i]); strcpy(tmp_args[i], arg[i]);
            tmp_args[i] = my_strdup(arg[i]);
        }

        // we want to memmove the string array entries now by count_diff to the
        // left. source should be end_args and dest should be end args - count
        // diff?
        ...
        //         fprintf(stderr, "\t NULL \n");
        //     } else {
        //         fprintf(stderr, "\targ[%i]: %s - hex dump: ", i,
        //         tmp_args[i]); print_hex_dump(tmp_args[i], strlen(tmp_args[i])
        //         + 1);
        //     }
        // }
    }
...
// instead of having to allocate everytime in a loop one can just call this
