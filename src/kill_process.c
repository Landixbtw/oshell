...
✅ 1. **Premature loop termination**: You have `break;` statements in both the
success and failure cases that cause the function to exit the loop after
checking only 1-2 processes instead of continuing through all `/proc/PID/`
directories.

✅ 2. **File size calculation problem**: You're using `ftell(fp)` immediately
after opening the file, but the file pointer is at position 0, so `ftell()`
returns 0. This means you're allocating almost no buffer space.

✅ 3. **Buffer reading issues**: The `fread()` approach with `sizeof(buffer)` as
parameters is problematic when `buffer` is a pointer.

(should be)
✅ 4. **Newline handling**: The `/proc/PID/comm` files contain a trailing
newline character that needs to be stripped for accurate string comparison.

✅ 5. **Logic error**: The `else if((strcmp(buffer, process_name_or_id) < 0) ||
(strcmp(buffer, process_name_or_id) > 0))` condition will always be true for
non-matching strings, causing an immediate break.

(should also be)
✅ 6. **Memory leaks**: `buffer` and `comm_file_path` aren't freed in all code
paths.

## Secondary Issues:

7. **Inconsistent error handling**: Some error paths don't properly clean up
allocated memory before exiting.

8. **Debugging confusion**: The current debug output makes it hard to see what's
actually being compared due to the newline issue.

The core problem is that your algorithm is sound, but the early termination and
file reading issues prevent it from working consistently. Fix those and it
...
int kill_process(char *process_name_or_id)
{
    ...
    if (string_to_int(process_name_or_id) == -1) {
        ...
        if (dir == NULL) {
            // no cleanup necessary here because nothing to clean if first thing
            // null
         ...
         * and check if the content matches the process name if yes, we have to
         * corresponding pid and we can kill the process
         * */
        ...
        while ((dirent = readdir(dir)) != NULL) {
            if (dirent->d_type == DT_DIR) {
                ...
                if (!is_numeric(dirent->d_name))
                    continue;

                // string length for proc_dir_name + dirent->d_name
                size_t full_proc_pathLength =
                    strlen(proc_dir_name) + strlen(dirent->d_name) + 1;
                ...
                if ((opendir(full_proc_path)) != NULL) {
                    ...
                    // visit we can "save" the dir name and convert + cast it to
                    // a pid so that if the entry is the correct one we have
                    // access to the pid and can kill the process via kill()
                    ...
                    if (comm_file_path == NULL) {
                        // perror("oshell: not able to allocate enough Memory
                        // for variable: 'comm_file_path'");
                        goto cleanup;
                    }
                    ...
                    // causes issues with using ftell. so this is seems to be a
                    // way to do it.
